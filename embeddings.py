...
class SingleDS(torch.utils.data.Dataset):
    ...
    def __init__(self, fns, get_x, get_y, vocab):
        self.fns,self.get_x,self.get_y,self.vocab = fns,get_x,get_y,vocab

    def __getitem__(self, i):
        return self.get_x(self.fns[i]), tensor(self.vocab[self.get_y(self.fns[i])])

    def __len__(self): return len(self.fns)

def get_all_embeddings(dl, m):
    with torch.no_grad():
        features_list = []
        y_list = []
        for x,y in tqdm(dl):
